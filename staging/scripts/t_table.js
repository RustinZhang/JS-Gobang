...
var T_Table = function () {
     ...
     */
    function T_Table() {
        (0, _classCallCheck3.default)(this, T_Table);

        this.trans_table = new _map2.default();
        this.hash_key = 0;
    ...
    (0, _createClass3.default)(T_Table, [{
        key: "calculateInitHashKey",
         ...
         */
        value: function calculateInitHashKey(board) {
            var stone = void 0;
            this.hash_key = 0;
            this.hash_checksum = 0;
            for (var i = 0; i < 15; i++) {
        ...
        key: "hashMakeMove",
        value: function hashMakeMove(entry, board) {
            var i = entry[0],
                j = entry[1],
                stone = board[i][j];
            this.hash_key += stone ? this.hash_lookup[i][j][1][0] : this.hash_lookup[i][j][0][0];
        ...
        key: "hashUnMakeMove",
        value: function hashUnMakeMove(entry, board) {
            var i = entry[0],
                j = entry[1],
                stone = board[i][j];
            this.hash_key -= stone ? this.hash_lookup[i][j][1][0] : this.hash_lookup[i][j][0][0];
        ...
        key: "lookUpHashTable",
        value: function lookUpHashTable(alpha, beta, depth) {
            var n = this.hash_key % this.size,
                entry = void 0;
        ...
        key: "enterHashTable",
        value: function enterHashTable(type, value, depth) {
            var n = this.hash_key % this.size;
            this.trans_table.set(n, [this.hash_checksum, type, value, depth]);
        }
    }], [{
        key: "zobristInit",
        value: function zobristInit() {
            var zobrist = [];
            for (var i = 15; i--;) {
                zobrist.push((0, _from2.default)({ length: 15 }));
            }
            for (var _i = 0; _i < 15; _i++) {
                for (var j = 0; j < 15; j++) {
                    zobrist[_i][j] = [[], []];
                    for (var k = 0; k < 2; k++) {
                        zobrist[_i][j][k][0] = Math.floor(Math.random() * 10e12);
                        zobrist[_i][j][k][1] = Math.floor(Math.random() * 10e12);
                    }
                }
            }
            return zobrist;
...
var t_table = exports.t_table = new T_Table();
//# sourceMappingURL=t_table.js.map
